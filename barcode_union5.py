...
width, height = 1280, 720  # 원하는 해상도 (예: 1280x720) 1920
...
while cap.isOpened():
    ...
    for i, detection in detections.iterrows():
        ...
        # ------------------------------------------------
        # (추가) 필터링된 마지막 바코드 이미지를 기반으로
        #        가운데 한 줄만 잘라서 위아래로 확장
        # ------------------------------------------------
        h, w, c = bilateral_img.shape
        center_line = bilateral_img[h // 2 : h // 2 + 1, :, :]  # 중앙 행(1줄) 추출
        expanded_barcode = np.repeat(center_line, h, axis=0)    # 높이만큼 반복 확장

        # 확장된 바코드 디코딩
        expanded_barcode_data = ""
        barcodes3 = pyzbar.decode(expanded_barcode)
        for barcode3 in barcodes3:
            expanded_barcode_data = barcode3.data.decode("utf-8")
            print(f"expanded_barcode: {expanded_barcode_data}")
            break
        
        # expanded_barcode 텍스트 상자 (x1+10, y2+90 위치)
        expanded_text = f"expanded_barcode: {expanded_barcode_data}" if expanded_barcode_data else "expanded_barcode:"
        expanded_text_size, expanded_baseline = cv2.getTextSize(expanded_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
        cv2.rectangle(frame, (x1 + 10, y2 + 90),
                      (x1 + 10 + expanded_text_size[0], y2 + 90 - expanded_text_size[1] - expanded_baseline),
                      (0, 255, 0), cv2.FILLED)
        cv2.putText(frame, expanded_text, (x1 + 10, y2 + 90 - expanded_baseline),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

        # 결과 영상 출력
        cv2.imshow("Expanded Barcode", expanded_barcode)
